...
class SignalClient:
    ...
    def _provide_signal_data(self, power_plant: str) -> pd.DataFrame:
        res = requests.get(f"{self._asset_api_base}/signals?limit={100000}&offset={0}&plant={power_plant}",
                           headers=self._auth_header)
        if res.status_code != 200:
            raise Exception(f"Data request failed with status code {res.status_code} "
                            f"and details: {res.json()}")
        signals = res.json()
        return pd.json_normalize(signals['records'])

    def _provide_power_plants(self) -> List[str]:
        res = requests.get(f"{self._asset_api_base}/assets?asset_type_identifier=power_plant",
                           headers=self._auth_header)
        if res.status_code != 200:
            raise Exception(f"Data request failed with status code {res.status_code} "
                            f"and details: {res.json()}")
    ...
    def provide_hash_table(self) -> pd.DataFrame:
        ...
        reduced_sensor_data_1["Unique"] = reduced_sensor_data['control_system_identifier'] + reduced_sensor_data['plant']
        reduced_sensor_data_2 = reduced_sensor_data_1.copy()
        reduced_sensor_data_2["Hash"] = reduced_sensor_data_1['Unique'].apply(hash)
        return pd.DataFrame(index=reduced_sensor_data_2["Hash"],
                            columns=['signal_id'],
